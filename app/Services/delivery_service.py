...
class DeliveryService:
    ...
    def mark_order_status(self, order_id, new_status):
        ...
        if new_status == "delivered": # has to verify OTP so we need to generate OTP 
            customer_email = self.order_repo.get_customerEmail_by_order_id(order_id) # get customer email
            otp_code = self.otp_service.generate_and_save_otp(customer_email) # message sent to customer
            if not otp_code:
